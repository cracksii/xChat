...
const loadGroups = async () => {
    ...
    groupIds.forEach(async id => {
        ...
        renderGroup(group.ID, group.name, `${lastMessageAuthor.name}: ${lastMessage.content}`, group.groupPicture);
    });
};
...
const loadMessages = async (groupID) => {
    ...
    if(messages.length === lastMessages.length && messages.every((value, index) => value === lastMessages[index]))  // Check if the groups have changed
        return;
    else
    ...
    messages.forEach(async messageID => {
        ...
        renderMessage(messageID, message.content, (author.ID === USER) ? "You": author.name, message.timestamp);
    });
};

const openChat = async (groupID) => {
    document.querySelector(".groups").setAttribute("style", "display: none")
    document.querySelector(".messageWrapper").setAttribute("style", "display: block");
    document.querySelector(".control").setAttribute("style", "display: flex");  
    document.querySelector("main").setAttribute("style", "padding-bottom: 10vh");
    
    let group = await request(`/group/${groupID}`, "GET");
    document.querySelector(".groupImage").src = group.groupPicture;
    document.querySelector(".groupInfo p").innerHTML = group.name;
    loadMessages(groupID);
};

const closeChat = () => {
    lastMessages = [];
    document.querySelector(".groups").setAttribute("style", "display: block")
    document.querySelector(".messageWrapper").setAttribute("style", "display: none");
...
const app = async () => {
    ...
    await request("/message/7", "DELETE");
};
...
document.querySelector(".sendButton").addEventListener("click", async () => {
    ...
    let msg = await request(`/messages/${currentGroupID}`, "POST", {
        "content": text
    });
    
    renderMessage(msg.ID, msg.content, "You", msg.timestamp);
